...
module TreeReader(V3f,int,int, int);

module Roots(V3f,int,int, int);
module Internode(InternodeData);
module Bud(BudData);
module PruneCap(float);
module Leaf(LeafData);


module Parab(V3f,float, float);
...
derivation length: 7 + 6 * 12;
Axiom: SB Box(false) EB SetColor(2) Orchard(5, 5);
...
TreeReader(pos,tn, rn, pn):
{
 ...
 while (!mtg.eof() && mtg.good()){
  ...
  if(foundStart){
    ...
    if(prevOrder-curOrder > 1){
     ...
     for(int o=curOrder+1;o<=prevOrder;o++){
      ...
      if(branchLevel[o] != -1){
       ...
       branchLevel[o]=-1;
       if(false){nproduce SB SetColor(41) Sphere(1.) EB;}
   ...
   if(curOrder > 0){
    ...
    for(int w = 0 ; w < 3 ; w++){
     if(newInternode.isLeaf && w < makeLeaf){
      ...
      nproduce SB RollL((360. / float(3))) Down(p) Leaf(LeafData(newInternode.node,curOrder,w,p,leafsize)) EB;
     }
     int nbh=nbranches.find(keepNode)->second;
     if(budCount < 3*1 -nbh){
      for(int bw=1;bw<=1;bw++){
       nproduce SB Down(((45.0) + (55.0 - 45.0) * float_rand())+((bw-1)*10.)) Bud(BudData(keepNode,curOrder,v[w],bw,true,activity,tn,rn,pn)) EB ;
      }
      budCount++;
     }
     nproduce RollL((360. / float(3)));
    }
   }
   ...
   if(LastNodeOfAxis[nodenum]==nodenum && branchLevel[curOrder] != -1){
    ...
    branchLevel[curOrder]=-1;
    if(false){nproduce SB SetColor(41) Sphere(1.) EB;}
    nproduce Bud(BudData(nodenum,curOrder,0,0,false,1.0,tn,rn,pn)) EB ;
   }
 ...
 for(int i=1;i<10;i++){
  if(branchLevel[i] > 0 ){
   ...
   nproduce Bud(BudData(0,i,0,0,false,1.0,tn,rn,pn)) EB;
  }
 }
 Printf("\n");
 mtg.close();
...
Bud(bd):
{
 ...
 if(bd.vigor > 2)
 {
  ...
  for(int i = 1 ; i <= numSegments ; i++)
  {
   ...
   for(int w = 0 ; w < 3 ; w++){
    float p=getNorm(64.,20.,3.,150.);
    nproduce SB RollR((360. / float(3))) Down(p) Leaf(LeafData(newInternode.node,bd.order,w,p,leafsize,0,bd.tn,bd.rn,bd.pn)) EB;
    for(int bw=1;bw<=1;bw++){
     nproduce SB Down(((45.0) + (55.0 - 45.0) * float_rand())+((bw-1)*10.)) Bud(BudData(highNode,bd.order,v[w],bw,true,activity,bd.tn,bd.rn,bd.pn))EB ;
    }
    nproduce RollL((360. / float(3)));
   }
  }
...
Leaf(ld) GetPos(px,py,pz) GetHead(hx,hy,hz):
{
 ...
 if(!ld.dead){
  ...
  if(ld.expose < 0.01f) {
   ...
   ld.fdead=flushno;
   if(false){
    nproduce SetHead(((-1.) + (1. - -1.) * float_rand()),0,((-1.) + (1. - -1.) * float_rand()),0,1,0);
   }
  }
 }
 produce Leaf(ld);
...
Roots(pos,tn,rn,pn):
{
 ...
 produce MoveTo3f(pos) Elasticity(0.1);
}
